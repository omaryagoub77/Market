...
export default function ChatRoomScreen() {
  ...
  const [notificationListeners, setNotificationListeners] = useState<any[]>([]);
  ...
  useEffect(() => {
    const listeners = expoNotificationService.setupNotificationListeners(
      (notification: any) => {
        ...
        console.log('Notification received in chat room:', notification);
        // You could show an in-app banner here if desired
      },
      (response: any) => {
        // Handle notification response (when user taps notification)
        console.log('Notification response received in chat room:', response);
        const data = response.notification.request.content.data;
        
        // Navigate to chat if notification contains chat data
        if (data?.senderId) {
          // Optionally navigate to the chat with this user
          // router.push(`/screens/chat-room?sellerId=${data.senderId}`);
        }
      }
    );
    ...
    return () => {
      expoNotificationService.removeNotificationListeners(listeners);
    };
  }, []);
  ...
  return (
    <ProtectedRoute redirectTo="/screens/chat-room">
      <NavWrapper>
        <ThemedView style={styles.container}>
